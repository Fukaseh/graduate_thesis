...
\subsection{?}
GSモデルについて，償却価値などの価格戦略を期間ごとに変更した場合のトレーダーの資産の分散の時間変化を図\ref{fig:GS_variance}(a)に，価格戦略を固定した場合の分散の時間変化を図\ref{fig:GS_variance}(b)に示す．
価格戦略を変更した場合は，グラフが直線的になっていることより分散は時間$t$に比例しており，diffusiveになっている．
...
\begin{figure}[htbp]
    ...
    \caption{(a)価格戦略を変更した場合の分散の時間変化, (b)価格戦略を固定した場合の分散の時間変化}
