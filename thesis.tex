...
そのため，$\alpha$と$\beta$が等しい値の場合，売注文の方が取引数が多くなりやすく，トレーダーの分布が偏ると考えられる．
実際$\alpha = 0.5, \beta = 0.5$におけるトレーダーの資産分布は図\のようになり，トレーダーの資産分布は偏っている．
注文は資産に比例して出されるため，資産分布が偏っているとき，資産変動も偏った値をとると考えられる．
$\beta$と比較して$\alpha$の値が小さいとき，トレーダーの売注文数と買注文数の比に偏りが生じない．
そのため，ファット・テール分布が現れる条件の1つとして売注文と買注文に偏りが少ないことが考えられる．
